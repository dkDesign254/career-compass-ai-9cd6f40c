    ...
    <AppShell>
      <div className="mx-auto max-w-4xl space-y-6">
        <Header
          ...
          desc="Paste your resume text — we'll score it for ATS systems and rewrite weak parts."
        />
          ...
          <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {uploading && (
                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Upload className="h-3 w-3 animate-pulse" /> Parsing…
                  </span>
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Copy-paste the contents of your CV here…"
              />
        ...
        {m.error && (
          <AiError
            error={m.error}
            onRetry={() => m.mutate({ resume_text: text, target_role: role || undefined })}
          />
        )}
          ...
          <div className="space-y-6">
            <Card>
              <CardContent className="flex items-center gap-8 p-6">
                ...
                <div className="flex-1 text-sm text-muted-foreground">
                  ATS systems prefer keyword density, clean structure, and quantified impact. Higher
                  is better — most strong resumes score 75+.
                </div>
              </CardContent>
            </Card>

            <div className="grid gap-4 md:grid-cols-2">
              <SectionCard title="Keyword matches">
                <ChipList items={m.data.keyword_matches} variant="secondary" />
              </SectionCard>
              <SectionCard title="Missing keywords">
                <ChipList items={m.data.missing_keywords} variant="outline" />
              </SectionCard>
            </div>

            <SectionCard title="Issues to fix">
              <ul className="space-y-1 text-sm">
                {m.data.issues.map((s: string, i: number) => (
                  <li key={i}>• {s}</li>
                ))}
              </ul>
            </SectionCard>
            ...
            <SectionCard title="Suggested bullets">
              <ul className="space-y-2 text-sm">
                {m.data.bullet_suggestions.map((s: string, i: number) => (
                  <li key={i}>• {s}</li>
                ))}
              </ul>
            </SectionCard>
