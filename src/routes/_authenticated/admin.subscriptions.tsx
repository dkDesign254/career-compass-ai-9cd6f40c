...
function SubsPage() {
  ...
  const grant = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ user_id: "", tier: "pro", months: 1 });
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        <Card>
          <CardContent className="grid gap-2 p-4 sm:grid-cols-[2fr_1fr_1fr_auto]">
            ...
            />
            <Button onClick={() => grant.mutate()} disabled={!form.user_id || grant.isPending}>
              Grant
            </Button>
          </CardContent>
        </Card>
                ...
                <CardContent className="flex items-center justify-between gap-3 p-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      <Badge>{s.tier}</Badge> · {s.status} · ends{" "}
                  ...
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      if (confirm("Revoke?")) revoke.mutate(s.user_id);
                    }}
                  >
                    Revoke
                  </Button>
