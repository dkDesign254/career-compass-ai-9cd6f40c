...
const PRESETS = {
  proceed:
    "We were impressed with your application and would like to invite you to the next round. Could you share your availability over the coming week?",
  shortlist:
    "Thanks for applying! You've been shortlisted — we're reviewing remaining candidates and will follow up shortly.",
  regret:
    "Thank you for the time you put into your application. After careful review we've decided to move forward with other candidates whose experience more closely matches the role. We wish you the best.",
};
...
function ApplicantsPage() {
  ...
  const [open, setOpen] = useState<null | {
    id: string;
    decision: "proceed" | "regret" | "shortlist";
    message: string;
  }>(null);
  const m = useMutation({
    mutationFn: () =>
      decideFn({
        data: { applicationId: open!.id, decision: open!.decision, message: open!.message },
      }),
    onSuccess: () => {
          ...
          <>
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {data.job.application_cap
                  ? ` · cap ${data.job.application_count}/${data.job.application_cap}`
                  : ""}
              </p>
            </div>
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          {a.resumes?.ats_score != null && (
                            <Badge variant="secondary">ATS {a.resumes.ats_score}</Badge>
                          )}
                    ...
                    <CardContent className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            setOpen({ id: a.id, decision: "shortlist", message: PRESETS.shortlist })
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setOpen({ id: a.id, decision: "proceed", message: PRESETS.proceed })
                          }
                        >
                        ...
                        <Button
                          ...
                          variant="destructive"
                          onClick={() =>
                            setOpen({ id: a.id, decision: "regret", message: PRESETS.regret })
                          }
                        >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <MessageSquare className="h-4 w-4" /> Send {open?.decision} message
              </DialogTitle>
            </DialogHeader>
            <Textarea
              rows={8}
