...
const words = (s?: string | null) => norm(s).split(/[^a-z0-9+.#]+/).filter(Boolean);
...
export function scoreJob(job: any, profile: any | null): JobScore {
  ...
  const prefTypes: string[] = Array.isArray(profile?.employment_types) ? profile.employment_types : [];
  if (prefModes.length) {
    alertTotal += 1;
    if (job?.work_mode && prefModes.includes(job.work_mode)) { alertHits += 1; reasons.push(`${job.work_mode} matches your preference`); }
  }
  if (prefTypes.length) {
    alertTotal += 1;
    if (job?.employment_type && prefTypes.includes(job.employment_type)) { alertHits += 1; reasons.push(`${job.employment_type} matches your preference`); }
  }
  ...
  if (salMin && job?.salary_max && job.salary_max >= salMin) { prefScore += 25; reasons.push("Salary meets your minimum"); }
  ...
  if (locs.length && job?.location && locs.some((l) => norm(job.location).includes(norm(l)))) {
    prefScore += 25; reasons.push(`Located in ${job.location}`);
  }
  const preference = Math.min(100, prefScore);

  const overall = Math.round(compatibility * 0.4 + bestFit * 0.3 + alertMatch * 0.15 + preference * 0.15);
