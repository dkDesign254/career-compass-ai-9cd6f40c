...
export function FeedbackThread({ applicationId, currentUserId }: { applicationId: string; currentUserId: string }) {
  const fetchThread = useServerFn(getThread);
  const sendFn = useServerFn(sendThreadMessage);
  const qc = useQueryClient();
  const queryKey = useMemo(() => ["thread", applicationId], [applicationId]);
  const { data, isLoading } = useQuery({ queryKey, queryFn: () => fetchThread({ data: { applicationId } }) });
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "feedback_messages", filter: `thread_id=eq.${data.thread.id}` }, () => {
  ...
  if (!data?.thread) return <p className="text-sm text-muted-foreground">No recruiter response yet.</p>;

  const decision = data.thread.decision;
  const badge = decision === "proceed" ? "default" : decision === "shortlist" ? "secondary" : "destructive";
  ...
  return (
    <div className="space-y-4">
        ...
        {data.messages.map((msg: any) => {
          ...
          return (
            <div key={msg.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
              <div className={cn("max-w-[80%] rounded-2xl px-4 py-2 text-sm", mine ? "bg-brand text-white" : "bg-background border")}>
      ...
      <div className="flex items-end gap-2">
        <Textarea rows={2} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Reply to the recruiter…" />
        <Button onClick={() => body.trim() && m.mutate(body.trim())} disabled={!body.trim() || m.isPending}>
